...
float Yaw1 = 0.0;
float rightDistance = 0.7;
float leftDistance = -0.7;
...
void imageCalllback(const sensor_msgs::ImageConstPtr& msg) 
{
  ...
  ////////////////////////////////////////////////////////////
    ...
    if(takeoff_flag && scan_adjust_flag && !cross_flag) {
...
void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
    ...
    {
      if(scanDistance[i]<0){
        right_angle = scanRange[i];
        rightDistance = scanDistance[i];
      }
      if(scanDistance[i]>0) {
        left_angle = scanRange[i];
        leftDistance = scanDistance[i];
      }
    }
